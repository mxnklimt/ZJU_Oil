...
#include <stdexcept>
#include <optional>
#include <chrono>          // 如果你用了 std::chrono 做超时判断
class RS485Manager {
public:
    ...
    //    return buffer;
    //}
    std::vector<uint8_t> receive(size_t expectedSize, DWORD timeoutMs = 15) {
        if (!isOpen()) {
            throw std::runtime_error("串口未打开");
        }
        ...
        while (buffer.size() < expectedSize) {
            if (!ReadFile(hSerial, tempBuf, sizeof(tempBuf), &bytesRead, NULL)) {
                throw std::runtime_error("接收数据失败");
            }
